    ...
    <div ref={dockRef} className={styles.dock}>
      <nav className={styles.rail} aria-label="Navigasi utama" onMouseLeave={scheduleClose}>
        <button
          ...
          style={reduce ? undefined : { animationDelay: '20ms' }}
        >
          <Command size={18} />
        ...
        {groups.map((group, i) => {
          ...
          const inner = (
            <>
              {isActive && (
                ...
                >
                  {/* Denyut satu kali saat mendarat di grup baru -- BUKAN napas
                      abadi. Kosakata "halo" yang sama dengan LastUpdatePill,
                      tapi dipakai untuk momen "tiba", bukan status yang hidup
                      terus-menerus, jadi ia harus berhenti sendiri. `key`
                      memutar ulang animasinya setiap kali grup aktif berganti. */}
                  {!reduce && <motion.span key={activeId} className={styles.pillPulse} initial={{ opacity: 0.6, scale: 0.7 }} animate={{ opacity: 0, scale: 1.7 }} transition={{ duration: 0.55, ease: [0.22, 1, 0.36, 1] }} />}
                </motion.span>
              )}
          ...
          const entrance = reduce ? undefined : { animationDelay: `${70 + i * 40}ms` };
          ...
          >
            ...
            {shown.links.map((link) => (
              ...
              >
                ...
                <ChevronRight size={14} className={styles.itemChevron} aria-hidden="true" />
              </Link>
            ))}
          </motion.div>
        )}
