  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, ease: [0.22, 1, 0.36, 1] }}>
      ...
      {loading ? (
        ...
        <>
          ...
          <MetricGrid
            ...
            icon={FileText}
            cards={[
              { key: 'pencatatan', icon: FileText, label: 'Realisasi Pencatatan', value: <AnimatedNumber value={contextRealisasiPencatatan} format={fmtRupiahDetail} />, accent: 'indigo' },
              { key: 'transaksional', icon: CreditCard, label: 'Realisasi Transaksional', value: <AnimatedNumber value={contextRealisasiTransaksional} format={fmtRupiahDetail} />, accent: 'purple' },
            ]}
          />
      ...
      <ExportDataModal
        ...
        title="Laporan Realisasi Penunjukan Langsung"
        filename={`Laporan_Penunjukan_Langsung_${new Date().toISOString().slice(0,10)}`}
