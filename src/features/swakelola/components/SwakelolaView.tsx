  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4 }}>
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ fontSize: 24, fontWeight: 600, margin: '0 0 4px', color: 'var(--text-primary)' }}>Realisasi Swakelola</h1>
              ...
              {currentData.map((p, i) => (
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
                    ...
                    <Badge variant={p.status === 'Paket Selesai' ? 'rendah' : 'sedang'} style={{ padding: '2px 6px', fontSize: 9 }}>{p.status}</Badge>
                  </div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Detail Swakelola">
        {selectedItem && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <div>
              <h3 style={{ margin: '0 0 8px', fontSize: 18, color: 'var(--text-primary)', lineHeight: 1.4 }}>{selectedItem.rup_name}</h3>
              <p style={{ fontSize: 13, color: 'var(--text-secondary)', margin: 0 }}>
                Tipe Swakelola: <strong style={{ color: 'var(--text-primary)' }}>{selectedItem.tipe_swakelola ? `Tipe ${selectedItem.tipe_swakelola}` : '-'} {selectedItem.kode_penyedia ? `(${selectedItem.kode_penyedia})` : ''}</strong>
              </p>
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, background: 'var(--bg-page)', padding: 16, borderRadius: 'var(--radius-lg)' }}>
              <div><span style={{ display: 'block', fontSize: 11, color: 'var(--text-tertiary)', marginBottom: 4 }}>Kode RUP</span><span style={{ fontFamily: 'var(--font-mono)', fontSize: 14, color: 'var(--text-primary)' }}>{selectedItem.kd_rup || selectedItem.rup_code || '-'}</span></div>
              <div><span style={{ display: 'block', fontSize: 11, color: 'var(--text-tertiary)', marginBottom: 4 }}>Metode Pengadaan</span><span style={{ fontFamily: 'var(--font-mono)', fontSize: 14, color: 'var(--text-primary)' }}>Swakelola</span></div>
              ...
              <div><span style={{ display: 'block', fontSize: 11, color: 'var(--text-tertiary)', marginBottom: 4 }}>Total Nilai Realisasi</span><span style={{ fontFamily: 'var(--font-mono)', fontSize: 14, color: 'var(--teal-600)', fontWeight: 600 }}>{fmtRupiah(selectedItem.total)}</span></div>
            </div>

            <div style={{ background: 'var(--bg-page)', padding: 16, borderRadius: 'var(--radius-lg)' }}>
              <h4 style={{ fontSize: 13, fontWeight: 600, margin: '0 0 12px', color: 'var(--text-primary)', borderBottom: '1px solid var(--border)', paddingBottom: 8 }}>Informasi Instansi & Satker</h4>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr' }}><span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>KLPD Utama</span><span style={{ fontSize: 12, color: 'var(--text-primary)', fontWeight: 500 }}>{selectedItem.nama_klpd || selectedItem.kode_klpd || '-'}</span></div>
                ...
                {selectedItem.nama_klpd_penyelenggara && <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr' }}><span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>KLPD Pny.</span><span style={{ fontSize: 12, color: 'var(--info-700)', fontWeight: 500 }}>{selectedItem.nama_klpd_penyelenggara}</span></div>}
                {selectedItem.nama_satker_penyelenggara && <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr' }}><span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>Satker Pny.</span><span style={{ fontSize: 12, color: 'var(--info-700)', fontWeight: 500 }}>{selectedItem.nama_satker_penyelenggara}</span></div>}
            ...
            <div style={{ background: 'var(--bg-page)', padding: 16, borderRadius: 'var(--radius-lg)' }}>
              <h4 style={{ fontSize: 13, fontWeight: 600, margin: '0 0 12px', color: 'var(--text-primary)', borderBottom: '1px solid var(--border)', paddingBottom: 8 }}>Waktu & Tanggal</h4>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                <div><span style={{ display: 'block', fontSize: 11, color: 'var(--text-tertiary)', marginBottom: 4 }}>Tgl Buat Paket</span><span style={{ fontSize: 13, color: 'var(--text-primary)', fontWeight: 500 }}>{selectedItem.tgl_buat_paket ? new Date(selectedItem.tgl_buat_paket).toLocaleString('id-ID') : '-'}</span></div>
                <div><span style={{ display: 'block', fontSize: 11, color: 'var(--text-tertiary)', marginBottom: 4 }}>Tgl Pengumuman</span><span style={{ fontSize: 13, color: 'var(--text-primary)', fontWeight: 500 }}>{selectedItem.tgl_pengumuman_paket ? new Date(selectedItem.tgl_pengumuman_paket).toLocaleString('id-ID') : '-'}</span></div>
                <div><span style={{ display: 'block', fontSize: 11, color: 'var(--text-tertiary)', marginBottom: 4 }}>Awal Kontrak</span><span style={{ fontSize: 13, color: 'var(--text-primary)', fontWeight: 500 }}>{selectedItem.tgl_awal_pelaksanaan_kontrak ? new Date(selectedItem.tgl_awal_pelaksanaan_kontrak).toLocaleDateString('id-ID') : '-'}</span></div>
                <div><span style={{ display: 'block', fontSize: 11, color: 'var(--text-tertiary)', marginBottom: 4 }}>Akhir Kontrak</span><span style={{ fontSize: 13, color: 'var(--text-primary)', fontWeight: 500 }}>{selectedItem.tgl_akhir_pelaksanaan_kontrak ? new Date(selectedItem.tgl_akhir_pelaksanaan_kontrak).toLocaleDateString('id-ID') : '-'}</span></div>
              </div>
            </div>

            <div style={{ background: 'var(--bg-page)', padding: 16, borderRadius: 'var(--radius-lg)' }}>
              <h4 style={{ fontSize: 13, fontWeight: 600, margin: '0 0 12px', color: 'var(--text-primary)', borderBottom: '1px solid var(--border)', paddingBottom: 8 }}>Detail Status</h4>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr' }}><span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>Status Paket</span><Badge variant={selectedItem.status === 'Paket Selesai' ? 'rendah' : 'sedang'} style={{ width: 'fit-content' }}>{selectedItem.status}</Badge></div>
                <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr' }}><span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>Status RUP</span><Badge variant={selectedItem.status_aktif_rup === true ? 'rendah' : 'default'} style={{ width: 'fit-content' }}>{selectedItem.status_aktif_rup === true ? 'Aktif' : 'Tidak / N/A'}</Badge></div>
              </div>
            </div>
